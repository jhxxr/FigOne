...
try:
...
except ImportError:  # Triton is optional for the bundled CPU runtime.
    triton = None
    ...
    class _TritonStub:
        language = _TritonLanguageStub()

        @staticmethod
        def jit(function):
            return function

    tl = _TritonStub.language
    triton = _TritonStub()
...
def edt_triton(data: torch.Tensor):
    ...
    if not data.is_cuda:
        try:
            ...
            values = data.detach().to("cpu").numpy().astype("uint8")
            distances = [
                cv2.distanceTransform(1 - image, cv2.DIST_L2, 0)
                for image in values
            ]
            return torch.as_tensor(distances, dtype=torch.float32, device=data.device)
        except ImportError:
            # Small CPU-only fallback for environments without OpenCV.
            result = torch.zeros(data.shape, dtype=torch.float32, device=data.device)
            for batch, image in enumerate(data):
                points = (~image).nonzero(as_tuple=False).to(torch.float32)
                if points.numel() == 0:
                    result[batch].fill_(float("inf"))
                    continue
                grid = torch.stack(torch.meshgrid(
                    torch.arange(data.shape[1], device=data.device),
                    torch.arange(data.shape[2], device=data.device),
                    indexing="ij",
                ), dim=-1).to(torch.float32).reshape(-1, 2)
                result[batch] = torch.cdist(grid, points).amin(dim=1).reshape(image.shape)
            return result
    assert triton is not None, "Triton is required for CUDA EDT"
    B, H, W = data.shape
