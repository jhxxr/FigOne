...
# ============================================================================
# LLM retry helpers (connection / transient failures only; no image compression)
# ============================================================================
...
def _llm_retry_settings(*, default_retries: int = 3, default_delay: float = 1.5) -> tuple[int, float]:
    """Resolve retry count/delay. Prefer FIGONE_LLM_* / FIGRA_LLM_*; fall back to OpenRouter envs."""
    retries_raw = os.environ.get("FIGONE_LLM_RETRIES", os.environ.get("FIGRA_LLM_RETRIES"))
    if retries_raw is None or not str(retries_raw).strip():
        retries = _env_int("OPENROUTER_MULTIMODAL_RETRIES", default_retries, minimum=1)
    else:
        retries = _env_int(
            "FIGONE_LLM_RETRIES" if "FIGONE_LLM_RETRIES" in os.environ else "FIGRA_LLM_RETRIES",
            default_retries,
            minimum=1,
        )

    delay_raw = os.environ.get("FIGONE_LLM_RETRY_DELAY", os.environ.get("FIGRA_LLM_RETRY_DELAY"))
    if delay_raw is None or not str(delay_raw).strip():
        retry_delay = _env_float("OPENROUTER_MULTIMODAL_RETRY_DELAY", default_delay, minimum=0.0)
    else:
        retry_delay = _env_float(
            "FIGONE_LLM_RETRY_DELAY" if "FIGONE_LLM_RETRY_DELAY" in os.environ else "FIGRA_LLM_RETRY_DELAY",
            default_delay,
            minimum=0.0,
        )
...
def _is_retryable_llm_error(exc: BaseException) -> bool:
    """Return True for transient network / 5xx style failures only."""
...
def _with_llm_retries(fn, *, label: str, retries: Optional[int] = None, delay: Optional[float] = None):
    """Run callable with exponential backoff on retryable LLM/network errors."""
    retry_count, retry_delay = _llm_retry_settings()
    ...
    for attempt in range(1, retry_count + 1):
        try:
            ...
            print(
                f"[{label}] 请求失败（尝试 {attempt}/{retry_count}，可重试）：{e}，"
                f"{sleep_s:.1f}s 后重试..."
            )
            time.sleep(sleep_s)
...
def _call_openai_compatible_text(
    ...
    try:
        return _with_llm_retries(_once, label="Custom/OpenAI-compatible text")
    except Exception as e:
...
def _call_openai_compatible_multimodal(
    ...
    """使用 OpenAI SDK 调用自定义 OpenAI 兼容多模态接口（全尺寸 PNG，不压缩）"""
    def _once() -> Optional[str]:
        from openai import OpenAI
    ...
    try:
        return _with_llm_retries(_once, label="Custom/OpenAI-compatible multimodal")
    except Exception as e:
...
# Multimodal request-only preview scales (does not affect disk artifacts / SVG canvas).
MULTIMODAL_IMAGE_SCALE_CHOICES = (1.0, 0.75, 0.5, 0.4, 0.25)
DEFAULT_MULTIMODAL_IMAGE_SCALE = 0.5


def normalize_multimodal_image_scale(value: Any, *, default: float = DEFAULT_MULTIMODAL_IMAGE_SCALE) -> float:
    """Validate and normalize a multimodal request image scale ratio."""
    ...
    try:
        ...
        raise ValueError(
            f"Invalid multimodal_image_scale `{value}`. "
            f"Expected one of: {', '.join(str(v) for v in MULTIMODAL_IMAGE_SCALE_CHOICES)}"
        ) from exc
    for choice in MULTIMODAL_IMAGE_SCALE_CHOICES:
        if abs(scale - choice) < 1e-9:
            return float(choice)
    raise ValueError(
        f"Unsupported multimodal_image_scale `{value}`. "
        f"Expected one of: {', '.join(str(v) for v in MULTIMODAL_IMAGE_SCALE_CHOICES)}"
    )
...
def prepare_multimodal_image(
    image: Image.Image,
    scale: float = DEFAULT_MULTIMODAL_IMAGE_SCALE,
    *,
    label: str = "image",
) -> Image.Image:
    """
    Create a request-only downsampled copy of a PIL image.

    - scale == 1.0 returns the original image object (no copy)
    - otherwise returns a new LANCZOS-resized image; source files are never touched
    """
    scale = normalize_multimodal_image_scale(scale)
    width, height = image.size
    if scale >= 1.0 or width <= 0 or height <= 0:
        print(f"  多模态预览图 {label}: 保持原尺寸 {width}x{height} (scale={scale:g})")
        return image

    new_width = max(1, int(round(width * scale)))
    new_height = max(1, int(round(height * scale)))
    # Avoid no-op resize when rounding collapses tiny deltas.
    ...
    print(
        f"  多模态预览图 {label}: {width}x{height} -> {new_width}x{new_height} "
        f"(scale={scale:g}, request-only)"
    )
    return resized


def _multimodal_preview_note(scale: float, figure_width: int, figure_height: int) -> str:
    """Prompt note so the model keeps original SVG coordinates after downsampling."""
    scale = normalize_multimodal_image_scale(scale)
    if scale >= 1.0:
        return ""
    return (
        "\n\nIMAGE TRANSPORT NOTE:\n"
        f"- Preview images attached to this request may be proportionally downsampled "
        f"(scale={scale:g}) for transport only.\n"
        f"- All SVG coordinates, width, height, and viewBox MUST still use the original "
        f"canvas: {figure_width} x {figure_height}.\n"
        "- Do NOT emit SVG dimensions matching the downsampled preview pixels.\n"
    )
...
def _call_openai_response_multimodal(
    ...
    try:
        return _with_llm_retries(_once, label="OpenAI Responses multimodal")
    except Exception as e:
...
def _call_openrouter_multimodal(
    ...
    def _once() -> Optional[str]:
        ...
        if response.status_code != 200:
            err = Exception(f'OpenRouter API 错误: {response.status_code} - {response.text[:500]}')
        ...
        message = choices[0].get('message', {})
        text_out = _extract_openrouter_message_text(message)
        if text_out:
            return text_out
    ...
    return _with_llm_retries(_once, label="OpenRouter multimodal")
...
def segment_with_sam3(
    ...
    if backend == "local":
        ...
        # Local checkpoint: FigOne-model/sam3.pt or Figra-model/sam3.pt (or SAM3_CHECKPOINT / AUTOFIGURE_SAM3_CHECKPOINT)
        checkpoint_candidates = []
        for env_key in ("SAM3_CHECKPOINT", "AUTOFIGURE_SAM3_CHECKPOINT"):
            env_val = os.environ.get(env_key)
            if env_val and env_val.strip():
                checkpoint_candidates.append(Path(env_val.strip()))
        # Prefer the sibling FigOne-model / Figra-model directory next to the project root.
        here = Path(__file__).resolve()
        project_root = here.parents[1]
        checkpoint_candidates.extend([
            project_root.parent / "FigOne-model" / "sam3.pt",
            project_root.parent / "Figra-model" / "sam3.pt",
            project_root / "FigOne-model" / "sam3.pt",
            project_root / "Figra-model" / "sam3.pt",
            here.parents[2] / "FigOne-model" / "sam3.pt" if len(here.parents) > 2 else project_root / "sam3.pt",
            Path.cwd() / "sam3.pt",
            Path.home() / "sam3.pt",
        ])
        ...
        if checkpoint_path is None:
            raise FileNotFoundError(
                "Local SAM3 checkpoint not found. Place sam3.pt under FigOne-model/ "
                "or set SAM3_CHECKPOINT / AUTOFIGURE_SAM3_CHECKPOINT."
            )
        ...
        # Adapt to current sam3.model_builder signature across versions
        try:
            ...
            if "checkpoint_path" in params:
                ...
                build_kwargs["checkpoint"] = str(checkpoint_path)
        ...
        # Drop None bpe_path if unsupported / optional
        if build_kwargs.get("bpe_path") is None:
            build_kwargs.pop("bpe_path", None)
        ...
        # SAM3's fused ViT MLP converts intermediate activations to BF16. Keep
        # the complete CUDA inference path in BF16 autocast so later Float32
        # linear layers receive compatible inputs, matching the official demos.
        inference_context = (
            torch.autocast(device_type="cuda", dtype=torch.bfloat16)
            if device == "cuda"
            else contextlib.nullcontext()
        )
        precision_mode = "CUDA BF16 autocast" if device == "cuda" else "CPU Float32"
        print(f"SAM3 推理精度: {precision_mode}")
        ...
        with inference_context:
            ...
            for prompt in prompt_list:
                print(f"\n  正在检测: '{prompt}'")
                # Ensure geometric/text state from the previous prompt does not
                # bleed into the next query (same pattern as official demos).
                ...
                if len(scores) == 0:
                    print(
                        f"  '{prompt}' 检测到 0 个有效对象 "
                        f"(processor 阈值 {processor_threshold} 下无候选)"
                    )
                    continue

                # Diagnostic: show top raw scores so empty/low runs are debuggable.
                try:
                    top_scores = sorted((float(s) for s in scores), reverse=True)[:5]
                    print(
                        f"  '{prompt}' 候选 {len(scores)} 个, "
                        f"top scores={[round(s, 3) for s in top_scores]}"
                    )
                except Exception:
                    pass
...
def _resolve_rmbg_model_path(rmbg_model_path: Optional[str] = None) -> Optional[str]:
    """Prefer an explicit path, then FigOne-managed local import, then HF cache/download."""
    candidates: list[str] = []
    if rmbg_model_path:
        candidates.append(rmbg_model_path)
...
def _ensure_rmbg2_access_ready(rmbg_model_path: Optional[str]) -> None:
    if _resolve_rmbg_model_path(rmbg_model_path):
        return
    ...
    if _has_rmbg2_cached_weights():
        return
    raise RuntimeError(
        "步骤三需要使用 briaai/RMBG-2.0，但当前未检测到本地权重。\n"
        "请到「模型与提供商」导入 model.safetensors（配置文件已内置），"
        "然后完全退出并重启 FigOne 再运行。"
    )
...
class BriaRMBG2Remover:
    ...
    def __init__(self, model_path: Path | str | None = None, output_dir: Path | str | None = None):
        resolved = _resolve_rmbg_model_path(str(model_path) if model_path else None)
        self.model_path = Path(resolved) if resolved else (Path(model_path) if model_path else None)
        ...
        if self.model_path and self.model_path.exists():
            ...
            try:
                ...
                if is_gated:
                    raise RuntimeError(
                        "无法从 HuggingFace 加载 RMBG-2.0（gated 模型）。\n"
                        "请到「模型与提供商」导入本地 model.safetensors，"
                        "然后完全退出并重启 FigOne 再运行。"
                    ) from e
                raise
...
def generate_svg_template(
    ...
    Args:
        ...
        multimodal_image_scale: 仅影响发给多模态模型的预览图比例，不改磁盘原图/SVG 画布
    """
    multimodal_image_scale = normalize_multimodal_image_scale(multimodal_image_scale)
    print("\n" + "=" * 60)
    ...
    preview_note = _multimodal_preview_note(multimodal_image_scale, figure_width, figure_height)
...
Please output ONLY the SVG code, starting with <svg and ending with </svg>. Do not include any explanation or markdown formatting."""
    ...
    request_figure = prepare_multimodal_image(
        figure_img, multimodal_image_scale, label="figure.png"
    )
    request_samed = prepare_multimodal_image(
        samed_img, multimodal_image_scale, label="samed.png"
    )
    contents = [prompt_text, request_figure, request_samed]
...
def svg_to_png(svg_path: str, output_path: str, scale: float = 1.0) -> Optional[str]:
    ...
    try:
        ...
        # 打包运行时通常没有 cairo 原生库，此时回退到纯 Python 渲染。
        print(f"  警告: cairosvg 无法加载原生库({e})，尝试使用其他方法")
    except Exception as e:
        print(f"  警告: cairosvg 转换失败: {e}，尝试使用其他方法")
...
def optimize_svg_with_llm(
    ...
    Args:
        ...
        multimodal_image_scale: 仅影响发给多模态模型的预览图比例
    ...
    for iteration in range(max_iterations):
        ...
        figure_width, figure_height = figure_img.size
        preview_note = _multimodal_preview_note(
            multimodal_image_scale, figure_width, figure_height
        )
...
- Keep the original canvas dimensions: width/height/viewBox must remain {figure_width} x {figure_height}
{preview_note}"""
        else:
            prompt = f"""You are an expert SVG optimizer. Compare the current SVG rendering with the original figure and optimize the SVG code to better match the original.
...
{preview_note}"""
        ...
        request_current = prepare_multimodal_image(
            current_png_img, multimodal_image_scale, label="current_svg.png"
        )
        contents = [prompt, request_figure, request_samed, request_current]
...
def load_icon_infos_from_output_dir(output_dir: str | Path, boxlib_path: str | Path) -> list[dict]:
    """Rebuild icon_infos from boxlib.json + icons/ without re-running RMBG."""
    output_dir = Path(output_dir)
    ...
    with open(boxlib_path, "r", encoding="utf-8") as f:
        boxlib_data = json.load(f)
    boxes = boxlib_data.get("boxes") or []

    icon_infos: list[dict] = []
    missing: list[str] = []
    for box_info in boxes:
        box_id = box_info["id"]
        label = box_info.get("label", f"<AF>{int(box_id) + 1:02d}")
        label_clean = label.replace("<", "").replace(">", "")
        x1, y1, x2, y2 = box_info["x1"], box_info["y1"], box_info["x2"], box_info["y2"]
        crop_path = icons_dir / f"icon_{label_clean}.png"
        nobg_path = icons_dir / f"icon_{label_clean}_nobg.png"
        if not _nonempty_file(crop_path) or not _nonempty_file(nobg_path):
    ...
    if missing:
        raise FileNotFoundError(
            "断点续跑缺少图标产物: " + ", ".join(missing)
            + f"（目录 {icons_dir}）。请删除不完整 icons 后从步骤 3 重跑，或去掉 --resume。"
        )
    return icon_infos


def detect_completed_through(
    output_dir: str | Path,
    *,
    optimize_iterations: int = 0,
) -> int:
    """
    Return highest fully completed pipeline step (0..5) based on artifacts.
    Step 4 requires template.svg; optimized_template.svg is only required when
    ...
    samed_path = output_dir / "samed.png"
    boxlib_path = output_dir / "boxlib.json"
    if not (_nonempty_file(samed_path) and _nonempty_file(boxlib_path)):
        return 1
    ...
    template_svg = output_dir / "template.svg"
    if not _nonempty_file(template_svg):
        return completed
    if optimize_iterations > 0 and not _nonempty_file(output_dir / "optimized_template.svg"):
        return completed
    completed = 4

    if _nonempty_file(output_dir / "final.svg"):
        return 5
    return completed
...
def method_to_svg(
    ...
    Args:
        ...
        start_from: 强制从指定步骤(1-5)开始重做（会覆盖该步及之后产物）
    ...
    if start_from is not None:
        if start_from not in (1, 2, 3, 4, 5):
            raise ValueError("start_from 必须是 1-5")
        resume = True

    completed_through = detect_completed_through(
        output_dir, optimize_iterations=optimize_iterations
    ) if resume else 0
    if start_from is not None:
        if completed_through < start_from - 1:
            raise ValueError(
                f"无法从步骤 {start_from} 开始：当前产物仅完成到步骤 {completed_through}。"
                "请先跑完前置步骤，或去掉 --start_from。"
            )
        resume_from_step = start_from
    elif resume:
        if completed_through >= stop_after:
            print(
                f"断点续跑：产物已完成到步骤 {completed_through}"
                f"（>= stop_after={stop_after}），无需继续。"
            )
            figure_path = output_dir / "figure.png"
            ...
            final = output_dir / "final.svg"
            icon_infos = []
            if _nonempty_file(boxlib):
                try:
                    with open(boxlib, "r", encoding="utf-8") as f:
                        boxes = (json.load(f) or {}).get("boxes") or []
                    if boxes and completed_through >= 3:
                        icon_infos = load_icon_infos_from_output_dir(output_dir, boxlib)
                except Exception:
                    icon_infos = []
            return {
                "figure_path": str(figure_path) if _nonempty_file(figure_path) else None,
                "samed_path": str(samed) if _nonempty_file(samed) else None,
                "boxlib_path": str(boxlib) if _nonempty_file(boxlib) else None,
                "icon_infos": icon_infos,
                "template_svg_path": str(template) if _nonempty_file(template) else None,
                "optimized_template_path": str(optimized) if _nonempty_file(optimized) else None,
                "final_svg_path": str(final) if _nonempty_file(final) else None,
                "resumed": True,
                "completed_through": completed_through,
            }
        resume_from_step = completed_through + 1
    else:
        resume_from_step = 1

    needs_step1_generation = resume_from_step <= 1 and input_figure_path is None
    if needs_step1_generation and not image_api_key:
        raise ValueError("必须提供 image_api_key（或复用 api_key）用于步骤一生图")
    if stop_after >= 4 and resume_from_step <= stop_after and not api_key:
        raise ValueError("步骤 4/5 需要提供 api_key")
    if resume_from_step <= 1 and input_figure_path is None and not method_text:
        raise ValueError("未提供 method_text，且未指定 input_figure_path（非 resume 或需重做步骤 1）")
    if resume and resume_from_step > 1 and not _nonempty_file(output_dir / "figure.png"):
        raise ValueError("断点续跑失败：缺少 figure.png")
    ...
    print(f"多模态预览缩放: {multimodal_image_scale:g}")
    if resume:
        print(f"断点续跑: 开启（已完成到步骤 {completed_through}，从步骤 {resume_from_step} 继续）")
    if start_from is not None:
        print(f"强制起始步骤: {start_from}")
    if not input_figure_path and resume_from_step <= 1:
    ...
    if resume_from_step > 1:
        print(f"\n跳过步骤 1：复用已有 {figure_path}")
    elif input_figure_path:
    ...
    if resume_from_step > 2:
        print(f"\n跳过步骤 2：复用已有 {samed_path} / {boxlib_path}")
        with open(boxlib_path, "r", encoding="utf-8") as f:
            valid_boxes = (json.load(f) or {}).get("boxes") or []
    else:
    ...
    if resume_from_step > 3:
        if no_icon_mode:
            print("跳过步骤 3：无图标模式（复用 boxlib）")
        else:
            print("跳过步骤 3：从 icons/ 重建 icon_infos")
            icon_infos = load_icon_infos_from_output_dir(output_dir, boxlib_path)
    elif no_icon_mode:
        ...
        icon_infos = crop_and_remove_background(
            ...
            boxlib_path=boxlib_path,
            output_dir=str(output_dir),
            rmbg_model_path=resolved_rmbg or rmbg_model_path,
        )
    ...
    try:
        if resume_from_step > 4:
            print(f"\n跳过步骤 4：复用已有 {template_svg_path}")
            if optimize_iterations > 0 and not _nonempty_file(optimized_template_path):
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--resume",
        action="store_true",
        help="从 --output_dir 已有产物断点续跑，跳过已完成的步骤",
    )
    parser.add_argument(
        "--start_from",
        type=int,
        choices=[1, 2, 3, 4, 5],
        default=None,
        help="强制从指定步骤开始重做（隐含 --resume，需已有前置产物）",
    )
    ...
    parser.add_argument(
        "--multimodal_image_scale",
        type=float,
        default=DEFAULT_MULTIMODAL_IMAGE_SCALE,
        choices=list(MULTIMODAL_IMAGE_SCALE_CHOICES),
        help=(
            "仅压缩发给多模态模型的预览图比例，不改 figure/samed/icons/最终 SVG 画布 "
            f"（可选: {', '.join(str(v) for v in MULTIMODAL_IMAGE_SCALE_CHOICES)}；"
            f"默认: {DEFAULT_MULTIMODAL_IMAGE_SCALE:g}）"
        ),
    )
    ...
    if not args.resume and args.start_from is None:
        if not args.method_text and not args.method_file and not args.input_figure_path:
            parser.error("必须提供 --method_text / --method_file / --input_figure_path 之一，或使用 --resume")
